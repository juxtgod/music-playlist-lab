...
let mySongs = [
  ...
  "Summer paradise",
  "Love song",
  "Wild thoughts", // Add your favorite song titles here as strings
  // Example: "Song Title"
];
...
function addSongToEnd() {
  ...
  if (mySongs.length === 0) {
    showMessage(
      "❌ Step 1 incomplete: Please add songs to mySongs array first"
    );
    return;
  }
  ...
  updateDisplays();
  showMessage(
    "✅ Added 'New Song' to the end using .push()! New length: " +
      mySongs.length
  );
}
...
function addSongToBeginning() {
  ...
  updateDisplays();
  showMessage(
    "✅ Added 'First Song' to the beginning using .unshift()! New length: " +
      mySongs.length
  );
}
...
function removeFirstSong() {
  ...
  updateDisplays();
  showMessage(
    "✅ Removed '" + removedSong + "' from the beginning using .shift()!"
  );
}
...
function createFormattedSongList() {
  ...
  showMessage(
    "✅ Created formatted song list using .map() and displayed with .forEach()!"
  );
}
...
function checkIfSongExists() {
  ...
  if (exists) {
    showMessage(
      "✅ Song '" +
        searchTerm +
        "' exists in your collection using .includes()!"
    );
  } else {
    showMessage(
      "❌ Song '" + searchTerm + "' does not exist using .includes()!"
    );
  }
}
...
function getTopThreeSongs() {
  ...
  showMessage(
    "✅ Top 3 songs using destructuring: 1) " +
      first +
      " 2) " +
      second +
      " 3) " +
...
function showMessage(text) {
  ...
  messagesDiv.innerHTML +=
    "<p><strong>" + currentTime + ":</strong> " + text + "</p>";
